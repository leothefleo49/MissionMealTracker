...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    insertCongregationSchema = createInsertSchema(congregations).pick({
      name: true,
      accessCode: true,
      stakeId: true
    });
    ...
    missionaries = pgTable("missionaries", {
      ...
      // NEW: Group number for duos/trios
    });
    ...
    insertMissionarySchema = createInsertSchema(missionaries).pick({
      ...
      groupNumber: true
      // NEW: Include groupNumber in the insert schema
    }).extend({
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/missionaries", async (req, res) => {
    try {
      ...
      console.log(`Successfully fetched ${missionaries2.length} missionaries.`);
  ...
  app2.get("/api/missionaries/:typeOrId", async (req, res) => {
    try {
      ...
      console.log(`Successfully fetched ${missionaries2.length} missionaries of type ${typeOrId}.`);
      res.json(missionaries2);
    } catch (err) {
  ...
  app2.get("/api/admin/missionaries/congregation/:congregationId", requireAdmin, async (req, res) => {
    try {
      ...
      console.log(`Successfully fetched ${missionaries2.length} missionaries for congregation ${parsedCongregationId}.`);
      res.json(missionaries2);
    } catch (err) {
  ...
  app2.get("/api/admin/congregations/:congregationId/users", requireAdmin, async (req, res) => {
    try {
      const { congregationId } = req.params;
      const parsedCongregationId = parseInt(congregationId, 10);
      if (isNaN(parsedCongregationId)) {
      ...
      if (!testMissionary) {
        try {
          const insertTestMissionary = {
            ...
            messengerAccount: "",
            groupNumber: null
            // Ensure groupNumber is handled for test missionary
          };
