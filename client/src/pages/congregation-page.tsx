...
export default function CongregationPage() {
  ...
  // Fetch congregation data by access code
  const { data: congregation, isLoading: loadingCongregation, error: congregationError } = useQuery({
  ...
  // Fetch missionaries for this congregation
  const { data: missionaries, isLoading: loadingMissionaries } = useQuery<any[]>({
    queryKey: ['/api/congregations', congregation?.id, 'missionaries'],
    queryFn: () => fetch(`/api/congregations/${congregation?.id}/missionaries`).then(res => res.json()),
    enabled: !!congregation?.id,
    staleTime: 1000,
  ...
  const { data: meals, isLoading: loadingMeals } = useQuery({
    queryKey: ['/api/congregations', congregation?.id, 'meals', startOfDay.toISOString(), sixMonthsLater.toISOString()],
    queryFn: () => fetch(
      `/api/meals?congregationId=${congregation?.id}&startDate=${startOfDay.toISOString()}&endDate=${sixMonthsLater.toISOString()}`
    ).then(res => res.json()),
    enabled: !!congregation?.id,
    staleTime: 1000,
    refetchInterval: 1000,
  ...
  // If invalid congregation access code, show error
  useEffect(() => {
    if (congregationError) {
      toast({
        title: "Invalid Access Link",
        description: "The congregation access link you're using is invalid or has expired. Please contact your congregation missionary coordinator.",
        variant: "destructive"
      });
    }
  }, [congregationError, toast]);
  ...
  // If congregation doesn't exist or is loading
  if (loadingCongregation) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <div className="text-center">
          <Calendar className="h-12 w-12 text-primary mx-auto mb-4" />
          <h1 className="text-xl font-bold mb-2">Loading Congregation Calendar...</h1>
          <p className="text-gray-500">Please wait while we load your congregation's meal calendar.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <div className="text-center max-w-md mx-auto">
          ...
          <h1 className="text-xl font-bold mb-2">Invalid Congregation Link</h1>
          <p className="text-gray-500 mb-6">
            The congregation link you're using is invalid or has expired. Please contact your congregation missionary coordinator for the correct link.
          </p>
            ...
            <div className="flex items-center min-w-0 flex-grow">
              ...
              <div className="ml-1 sm:ml-2 min-w-0 flex-grow">
                <h1 className="text-sm sm:text-xl font-bold text-gray-900 truncate">
                  {isMobile ? congregation.name : `${congregation.name} - Missionary Meal Calendar`}
                </h1>
                {!isMobile && (
                  <p className="text-sm text-gray-500">
                    Schedule meals for missionaries serving in the {congregation.name}
                  </p>
                )}
              </div>
            </div>
          ...
          <Tabs defaultValue="calendar" value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="ml-3">
                        ...
                        <div className="mt-2 text-sm text-yellow-700">
                          <p>
                            {missionaries?.find(m => m.id.toString() === missionaryType)?.name || 'This missionary'}
                            {' '}is already booked for a meal on {selectedDate.toLocaleDateString()}.
                            Please select a different date or choose another missionary.
                          </p>
                        </div>
                        <div className="mt-3">
                          <button
            ...
            <TabsContent value="contact">
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-600">
                  Here you can find contact information for the missionaries serving in {congregation.name}.
                </p>
              </div>
              ...
              {loadingMissionaries ? (
                ...
                <div className="text-center py-8">
                  <p>No active missionaries found for this congregation.</p>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="statistics">
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-600">
                  View comprehensive meal data, missionary frequency, and trends for your congregation.
                </p>
              </div>

              {congregation?.id && <MealStatistics congregationId={congregation.id} />}
            </TabsContent>
          </Tabs>
        </div>
      </main>
      ...
      <footer className="bg-white mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col items-center justify-between md:flex-row">
            <div className="flex items-center">
              <p className="text-sm text-gray-500">&copy; {new Date().getFullYear()} {congregation?.name} - Missionary Meal Calendar</p>
            </div>
          </div>
        </div>
      </footer>
