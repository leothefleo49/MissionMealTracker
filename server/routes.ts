...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const notifyMissionary = async (missionaryId: number, message: string) => {
    ...
    if (
      ...
      console.log(
        `Cannot send SMS to ${missionary.name}: Consent status is ${missionary.consentStatus}`,
      );
      return false;
    }
    ...
    if (
      missionary.preferredNotification === "messenger" &&
      !missionary.messengerAccount
    ) {
      console.log(
        `Cannot send messenger notification to ${missionary.name}: No messenger account provided`,
      );
      return false;
    }
    ...
    try {
      ...
      return await notificationManager.sendCustomMessage(
        missionary,
        message,
        "status_update",
      );
  ...
  app.get("/api/missionaries/:typeOrId", async (req, res) => {
    try {
      ...
      if (typeOrId !== "elders" && typeOrId !== "sisters") {
        return res
          .status(400)
          .json({ message: 'Type must be either "elders" or "sisters"' });
      }
  ...
  app.get(
    "/api/admin/missionaries/ward/:wardId",
    requireAdmin,
    async (req, res) => {
      try {
        ...
        if (!req.user!.isSuperAdmin) {
          ...
          if (!hasAccess) {
            return res
              .status(403)
              .json({ message: "You do not have access to this ward" });
          }
        }
  ...
  app.post("/api/meals/check-availability", async (req, res) => {
    try {
      ...
      const isAvailable = await storage.checkMealAvailability(
        date,
        data.missionaryType,
        wardId,
      );
  ...
  app.post("/api/meals", async (req, res) => {
    try {
      ...
      const isAvailable = await storage.checkMealAvailability(
        mealDate,
        mealData.missionaryId.toString(),
        wardId,
      );
      ...
      if (
        ...
        if (missionary.consentStatus === "granted") {
          ...
          const shouldResendVerification =
            !missionary.consentVerificationSentAt ||
            new Date().getTime() -
              new Date(missionary.consentVerificationSentAt).getTime() >
              24 * 60 * 60 * 1000;
          ...
          if (shouldResendVerification) {
            ...
            if (
              ...
              notificationManager.smsService.twilioClient
            ) {
              ...
              console.log(
                `Consent verification sent to missionary ${missionary.id} (${missionary.name})`,
              );
            } else {
              // Fallback for development without Twilio
              console.log(
                `[SMS CONSENT REQUEST] Would send to ${missionary.phoneNumber}: ${consentMessage}`,
              );
            }
          }
        } else if (
          !missionary.consentStatus ||
        ...
        if (updateData.cancelled) {
          ...
          await notifyAdmin(
            `Meal cancelled: ${formattedDate} at ${updatedMeal.startTime} with ${updatedMeal.hostName} for ` +
              `missionary ${
                missionary ? missionary.name : `ID ${updatedMeal.missionaryId}`
              }. ` +
              (updateData.cancellationReason
                ? `Reason: ${updateData.cancellationReason}`
                : ""),
          ...
          await notifyMissionary(
            ...
            `Meal updated: ${formattedDate} at ${updatedMeal.startTime} with ${updatedMeal.hostName}. ` +
              (updatedMeal.mealDescription
                ? `Menu: ${updatedMeal.mealDescription}`
                : "") +
              (updatedMeal.specialNotes
                ? ` Notes: ${updatedMeal.specialNotes}`
                : ""),
          );
        }
  ...
  app.post("/api/meals/:id/cancel", async (req, res) => {
    try {
      ...
      if (cancelledMeal) {
        // Get the missionary to check consent status
        const missionary = await storage.getMissionary(
        ...
        await notifyAdmin(
          `Meal cancelled: ${formattedDate} at ${cancelledMeal.startTime} with ${cancelledMeal.hostName} for ` +
            `missionary ${
              missionary ? missionary.name : `ID ${cancelledMeal.missionaryId}`
            }. ` +
            (reason ? `Reason: ${reason}` : ""),
        );
  ...
  app.patch("/api/admin/missionaries/:id", requireAdmin, async (req, res) => {
    try {
      ...
      if (
        ...
        req.body.emailAddress !== existingMissionary.emailAddress
      ) {
      ...
      const updatedMissionary = await storage.updateMissionary(
        missionaryId,
        req.body,
      );
  ...
  app.post(
    "/api/admin/missionaries/:id/send-verification",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
        ...
        if (!missionary.emailAddress.endsWith("@missionary.org")) {
          return res
            .status(400)
            .json({ message: "Email must end with @missionary.org" });
        }
  ...
  app.post(
    "/api/admin/missionaries/:id/verify-email",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
        ...
        const success = await emailVerificationService.verifyCode(
          missionaryId,
          code,
        );
  ...
  app.post("/api/missionary-portal/authenticate", async (req, res) => {
    try {
      ...
      if (!missionary.password) {
        return res
          .status(401)
          .json({ authenticated: false, message: "Password not set. Please register first." });
      }
      ...
      res.json({
        authenticated: true,
        missionary: { id: missionary.id, name: missionary.name },
      });
    } catch (error) {
  ...
  app.post("/api/missionaries/register", async (req, res) => {
    try {
      ...
      if (!emailAddress.endsWith("@missionary.org")) {
        return res
          .status(400)
          .json({ message: "Email must be a @missionary.org address" });
      }
      ...
      if (existingMissionary) {
        ...
        if (!existingMissionary.password) {
          ...
          return res
            .status(409)
            .json({ message: "Missionary already registered with this email" });
        }
      } else {
  ...
  app.post("/api/missionaries/verify", async (req, res) => {
    try {
      ...
      if (!missionaryId || !verificationCode) {
        return res
          .status(400)
          .json({ message: "Missionary ID and verification code are required" });
      }
  ...
  app.post("/api/wards/:wardId/leave", requireAuth, async (req, res) => {
    try {
      ...
      if (success) {
        ...
        res
          .status(404)
          .json({ message: "Ward not found or user not a member" });
      }
    } catch (error) {
  ...
  app.get("/api/admin/stats", requireAdmin, async (req, res) => {
    try {
      ...
      const wardId = req.query.wardId
        ? parseInt(req.query.wardId as string, 10)
        : undefined;
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        const success = await storage.removeUserFromWard(
          parsedUserId,
          parsedWardId,
        );
        ...
        if (success) {
          ...
          res
            .status(404)
            .json({ message: "User-ward relationship not found" });
        }
      } catch (err) {
  ...
  app.get("/api/wards/:accessCode", async (req, res) => {
    try {
      ...
      if (!ward.active) {
        return res
          .status(403)
          .json({ message: "This ward is no longer active" });
      }
  ...
  app.get("/api/message-stats", requireAdmin, async (req, res) => {
    try {
      const wardId = req.query.wardId
        ? parseInt(req.query.wardId as string)
        : undefined;
      ...
      if (wardId) {
        ...
        if (!req.user!.isSuperAdmin) {
          return res
            .status(403)
            .json({
              message: "Access to all stats requires super admin privileges",
            });
        }
  ...
  app.post("/api/admin/test-message", requireAdmin, async (req, res) => {
    try {
      ...
      if (
        !notificationMethod ||
        !["email", "whatsapp"].includes(notificationMethod)
      ) {
        return res
          .status(400)
          .json({
            message: "Valid notification method (email or whatsapp) is required",
          });
      }
      ...
      if (messageType === "custom" && !customMessage) {
        return res
          .status(400)
          .json({ message: "Message text is required for custom messages" });
      }
      ...
      if (schedulingOption === "scheduled" && (!scheduledDate || !scheduledTime)) {
        return res
          .status(400)
          .json({ message: "Date and time are required for scheduled messages" });
      }
      ...
      if (!req.user!.isSuperAdmin) {
        ...
        if (!hasAccess) {
          return res
            .status(403)
            .json({ message: "Access denied for this ward" });
        }
      }
      ...
      let testMissionary = await storage.getMissionaryByName(
        wardId,
        "Test Missionary",
      );
      ...
      if (!testMissionary) {
        try {
          const insertTestMissionary = {
            ...
            phoneNumber:
              notificationMethod === "whatsapp" ? contactInfo : "+15551234567",
            emailAddress:
              notificationMethod === "email" ? contactInfo : "test@missionary.org",
            whatsappNumber:
              notificationMethod === "whatsapp" ? contactInfo : null,
          ...
          return res
            .status(500)
            .json({ message: "Failed to create test missionary" });
        }
      } else {
      ...
      console.log(
        `Test message will be sent via ${notificationMethod} to ${contactInfo}`,
      );

      // For scheduled messages, add to a queue (just mock this for now)
      if (schedulingOption === "scheduled") {
        console.log(
          `Test message scheduled for ${scheduledDate} at ${scheduledTime}`,
        );
        result = true;
      } else {
        ...
        switch (messageType) {
          case "meal_reminder":
            if (!mockMeal) {
              return res
                .status(400)
                .json({ message: "Meal details are required for meal reminders" });
            }
            result = await notificationManager.sendMealReminder(
              mockMissionary as any,
              mockMeal as any,
            );
            break;
          ...
          case "day_of":
            if (!mockMeal) {
              return res
                .status(400)
                .json({
                  message: "Meal details are required for day-of reminders",
                });
            }
            result = await notificationManager.sendDayOfReminder(
              mockMissionary as any,
              [mockMeal as any],
            );
            break;
          ...
          case "weekly_summary":
            if (!mockMeal) {
              return res
                .status(400)
                .json({
                  message: "Meal details are required for weekly summaries",
                });
            }
            result = await notificationManager.sendWeeklySummary(
              mockMissionary as any,
              [mockMeal as any],
            );
            break;
            ...
            if (!customMessage) {
              ...
              if (mockMeal) {
                ...
                return res
                  .status(400)
                  .json({
                    message: "Message text is required for custom messages",
                  });
              }
            } else {
              try {
      ...
      if (result) {
        res
          .status(200)
          .json({ success: true, message: "Test message sent successfully" });
      } else {
        res
          .status(500)
          .json({ success: false, message: "Failed to send test message" });
      }
    } catch (error) {
  ...
  app.post(
    "/api/missionaries/:id/request-consent",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
        ...
        try {
          if (
            testMissionary.preferredNotification === "messenger" &&
            testMissionary.messengerAccount
          ) {
            // For messenger notifications
            console.log(
              `[MESSENGER CONSENT REQUEST] Sending to ${testMissionary.messengerAccount}: ${consentMessage}`,
            );
            success = true;
          } else {
            ...
            if (
              ...
              // Fallback for development without Twilio
              console.log(
                `[SMS CONSENT REQUEST] Sending to ${testMissionary.phoneNumber}: ${consentMessage}`,
              );
              success = true;
            }
          }
  ...
  app.post("/api/sms/webhook", async (req, res) => {
    try {
      ...
      if (!missionary) {
        console.log(
          "Missionary not found with exact phone number match, trying alternative formats...",
        );

        // Try without the + prefix
        const numberWithoutPlus = phoneNumber.startsWith("+")
          ? phoneNumber.substring(1)
          : phoneNumber;
        missionary = missionaries.find(
          (m) =>
            m.phoneNumber === numberWithoutPlus ||
            (m.phoneNumber.startsWith("+") &&
              m.phoneNumber.substring(1) === numberWithoutPlus),
        );

        if (missionary) {
          console.log(
            `Found missionary ${missionary.id} using alternative phone format matching`,
          );
        }
      }
      ...
      console.log(
        `Missionary ${missionary.id} verification token: ${missionary.consentVerificationToken}`,
      );
        ...
        if (parts.length >= 2) {
          ...
          if (code === missionary.consentVerificationToken) {
            ...
            const confirmMessage =
              "Thank you! You have successfully opted in to receive meal notifications. Reply STOP at any time to opt out.";
            ...
            try {
              if (
                ...
                console.log(
                  `[MESSENGER CONFIRMATION] Sending to ${testMissionary.messengerAccount}: ${confirmMessage}`,
                );
              } else {
                ...
                if (
                  ...
                  console.log(
                    `[SMS CONFIRMATION] Sending to ${testMissionary.phoneNumber}: ${confirmMessage}`,
                  );
                }
              }
            } catch (error) {
  ...
  app.get(
    "/api/missionaries/:id/consent-status",
    requireAdmin,
    async (req, res) => {
      try {
        const { id } = req.params;
  ...
  app.get("/api/meal-stats/:wardId", async (req, res) => {
    try {
      ...
      const timeRangeInWeeks = Math.max(
        1,
        Math.ceil(
          (endDate.getTime() - startDate.getTime()) / (7 * 24 * 60 * 60 * 1000),
        ),
      );
      const timeRangeInMonths = Math.max(
        1,
        Math.ceil(
      ...
      const missionaryMealCounts = new Map<
        number,
        { count: number; lastMeal: Date | null }
      >();

      activeMeals.forEach((meal) => {
        const current = missionaryMealCounts.get(meal.missionaryId) || {
          count: 0,
          lastMeal: null,
        };
        current.count++;
      ...
      const missionaryStats = missionaries
        .map((missionary) => ({
          ...
          lastMeal:
            missionaryMealCounts.get(missionary.id)?.lastMeal?.toISOString() ||
            null,
        }))
      ...
      activeMeals.forEach((meal) => {
        const mealDate = new Date(meal.date);
        const monthKey = mealDate.toLocaleDateString("en-US", {
          year: "numeric",
          month: "short",
        });
  ...
  app.post("/api/missionary-forgot-password", async (req, res) => {
    try {
      ...
      if (!emailAddress || !accessCode) {
        return res
          .status(400)
          .json({ message: "Email address and access code are required" });
      }
      ...
      if (!emailAddress.endsWith("@missionary.org")) {
        return res
          .status(400)
          .json({ message: "Please use your @missionary.org email address" });
      }
      ...
      if (!missionary || missionary.wardId !== ward.id) {
        return res
          .status(404)
          .json({ message: "Missionary not found in this ward" });
      }
  ...
  app.post("/api/missionary-change-password", async (req, res) => {
    try {
      const { accessCode, emailAddress, currentPassword, newPassword } =
        req.body;
      ...
      if (!missionary.password) {
        return res
          .status(401)
          .json({ message: "No password set. Please contact administrator." });
      }
